...
def remove_inside(path):
    if os.path.exists(os.path.join(path, "annotations")):
        shutil.rmtree(os.path.join(path, "annotations"), ignore_errors=True)
    if os.path.exists(os.path.join(path, "annotations", "xmls")):
        shutil.rmtree(os.path.join(path, "annotations", "xmls"), ignore_errors=True)
...
def transform(input_folder, output_folder, folder_ann, folder_img, display_mode):
    ...
    try:
        ...
        os.mkdir(os.path.join(output_folder, 'annotations', 'xmls'))
    ...
    _file = open(os.path.join(trainval_path, 'trainval.txt'), 'w')
